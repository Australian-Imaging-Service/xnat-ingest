...
def _disconnect_session(session: "xnat.session.XNATSession") -> None:
    """Best-effort close of an xnatpy XNATSession used during --loop iterations.

    Errors during disconnect must not mask a real exception propagating up the
    stack, so they are logged at debug level only. Called via ExitStack.callback
    in upload() to guarantee cleanup even if the function raises."""
    try:
        session.disconnect()
    except Exception as exc:  # noqa: BLE001 — best-effort cleanup
        logger.debug("Ignoring error during XNATSession disconnect: %s", exc)
...
def upload(
    ...
    # Per-call resource cleanup. cli/upload.py wraps this function in a
    # `while True:` loop when --loop is set, so without explicit cleanup
    # every iteration leaked:
    #   * the tempdir passed as Xnat(cache_dir=...)
    #   * the tempdir passed to iterate_s3_sessions(s3_cache_dir=...)
    #   * the xnat.connect()'d XNATSession when use_curl_jsession is set
    # Over hours of operation this exhausted host memory and triggered
    # global OOM cascades on the deployment node. ExitStack guarantees
    # cleanup runs even if the function raises.
    ...
    try:
        ...
        if use_curl_jsession:
            ...
            # The manually-assigned XNATSession at .session is NOT closed by
            # `with xnat_repo.connection:` below — that context manager only
            # operates on xnat_repo.connection itself. Register disconnect()
            # to run on ExitStack unwind so the urllib3 connection pool and
            # xnatpy object cache are released between --loop iterations.
            cleanup.callback(_disconnect_session, xnat_repo.connection.session)
